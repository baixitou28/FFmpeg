...
// Write memory barrier needed for SMP here in theory
        ...
        wndx    += len;
        size    -= len;
    } while (size > 0);//如果还要写，可能需要等待读清空fifo，所以这里就无条件循环阻塞了==>一般写一帧，读一帧的节奏也是会阻塞的，
...
// memory barrier needed for SMP here in theory
    ...
    } while (buf_size > 0);//举例：原始的buf_size = sizeof(AVFrame) ， buf_size>0说明还没有读全  ==>真的会读不全吗？读一帧，也可能没数据，如果是读多帧，概率更大，fifo空了。这里是无条件阻塞
    return 0;
}
