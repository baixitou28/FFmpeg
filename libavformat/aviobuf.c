    ...
    while (size > 0) {
        ...
        if (len == 0 || s->write_flag) {//如果长度为0  ==>
            if((s->direct || size > s->buffer_size) && !s->update_checksum) {
                ...
                fill_buffer(s);///如果是普通文件，走这个分支，填充数据 fill_buffer-->read_packet_wrappers-> s->read_packet(io_read_packet) -->ffurl_read-->retry_transfer_wrapper(h->prot->url_read==file_read, h->proto==ff_file_protocol) -->file_read
                len = s->buf_end - s->buf_ptr;
                if (len == 0)
                    break;
            }
        } else {
            memcpy(buf, s->buf_ptr, len);//如果是普通文件，如果缓存有数据，走这个分支，直接复制
            buf += len;
            s->buf_ptr += len;
            size -= len;
        }
