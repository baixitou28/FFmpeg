...
#define AVPROBE_SCORE_RETRY (AVPROBE_SCORE_MAX/4)//一般是25
...
#define AVFMT_TS_DISCONT    0x0200 /**< Format allows timestamp discontinuities. Note, muxers always require valid (monotone) timestamps *///tiger PROGRAM 是否很需要？
...
#define MAX_STD_TIMEBASES (30*12+30+3+6)//tiger 
    ...
    enum AVStreamParseType need_parsing;//举例 adts_aac_read_header: st->need_parsing         = AVSTREAM_PARSE_FULL_RAW;
    struct AVCodecParserContext *parser;//举例：read_frame_internal  st->parser = av_parser_init(st->codecpar->codec_id);
