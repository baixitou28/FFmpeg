...
static int insert_trim(int64_t start_time, int64_t duration,
                       AVFilterContext **last_filter, int *pad_idx,//是否要跳过某个时间,如果是插入一个trim 的AVFilter，
...
static int configure_output_video_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)//一般第一个是ff_vf_null， 创建buffersink为最后一个filter
{//ff_vf_null: 在这里生成 ffmpeg_parse_options->open_files-->open_output_file-->new_video_stream->get_ost_filters
    char *pix_fmts;
    OutputStream *ost = ofilter->ost;
    OutputFile    *of = output_files[ost->file_index];
    AVFilterContext *last_filter = out->filter_ctx;//null即ff_vf_null是第一个
    int pad_idx = out->pad_idx;
    int ret;
    char name[255];
    //01. 用buffersink创建一个filter，和相关的AVFilterContext，插入 fg->graph->filters
    snprintf(name, sizeof(name), "out_%d_%d", ost->file_index, ost->index);
    ret = avfilter_graph_create_filter(&ofilter->filter,////第一个直接创建在ofilter->filter上，所有没有使用avfilter_link //tiger 01 ifilter_send_frame-->configure_filtergraph->configure_output_video_filter-->avfilter_graph_create_filter-->avfilter_graph_alloc_filter-->avfilter_graph_alloc_filter
                                       avfilter_get_by_name("buffersink"),//tiger 02 先创建"buffersink" 的filter
    ...
    if (ofilter->width || ofilter->height) {//未指定，就不一定有
        char args[255];
        ...
        if ((ret = avfilter_graph_create_filter(&filter, avfilter_get_by_name("scale"),//查找scale的AVfilter，并创建Context，插入fg->graph
                                                name, args, NULL, fg->graph)) < 0)
            return ret;
        if ((ret = avfilter_link(last_filter, pad_idx, filter, 0)) < 0)////将last_filter做为src， filter作为dst，串起来，这样buffersrc放前面，scale放后面
            return ret;
    ...
    if ((pix_fmts = choose_pix_fmts(ofilter))) {
        ...
        ret = avfilter_graph_create_filter(&filter,
                                           ...
                                           "format", pix_fmts, NULL, fg->graph);//插入 filter format
        av_freep(&pix_fmts);
        if (ret < 0)
            return ret;
    ...
    if (ost->frame_rate.num && 0) {
        ...
        snprintf(name, sizeof(name), "fps_out_%d_%d",
                 ost->file_index, ost->index);
        ret = avfilter_graph_create_filter(&fps, avfilter_get_by_name("fps"),//插入 filter fps
                                           name, args, NULL, fg->graph);
        if (ret < 0)
            return ret;

        ret = avfilter_link(last_filter, pad_idx, fps, 0);//fps是放后面的
        if (ret < 0)
            return ret;
        last_filter = fps;
        pad_idx = 0;
    ...
    snprintf(name, sizeof(name), "trim_out_%d_%d",
             ost->file_index, ost->index);
    ret = insert_trim(of->start_time, of->recording_time,//05.从哪帧开始和结束， 插入 filter trim
                      &last_filter, &pad_idx, name);
    if (ret < 0)
        return ret;

    //06. 特别注意：前面都是以ff_vf_null为头，相互串起来的，最后才是将ofilter->filter即buffersink 放在末尾
    if ((ret = avfilter_link(last_filter, pad_idx, ofilter->filter, 0)) < 0)//trim 是放在其他前面的
        return ret;

    return 0;
}

static int configure_output_audio_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)//一般第一个是ff_af_anull， 创建abuffersink为最后一个filter
{
    OutputStream *ost = ofilter->ost;
    OutputFile    *of = output_files[ost->file_index];
    ...
    char name[255];
    int ret;
    //01. 创建abuffersink 的一个filterContext，插入fg->graph->filters里
    snprintf(name, sizeof(name), "out_%d_%d", ost->file_index, ost->index);//自定义的名称out_xxxxx
    ret = avfilter_graph_create_filter(&ofilter->filter,//第一个直接创建在ofilter->filter上，所有没有使用avfilter_link
...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg) do {                 \
    ...
    if (ost->audio_channels_mapped) {//02.map_channel 参数
        int i;
        AVBPrint pan_buf;
        ...
        AUTO_INSERT_FILTER("-map_channel", "pan", pan_buf.str);//插入filter pan， ff_af_anull->pan
    ...
    if (sample_fmts || sample_rates || channel_layouts) {//04. 如果有音频参数
        ...
        snprintf(name, sizeof(name), "format_out_%d_%d",//打印成类似:"sample_fmts=fltp:sample_rate=96000|88200|....|8000|7350"
                 ost->file_index, ost->index);
        ret = avfilter_graph_create_filter(&format,
                                           avfilter_get_by_name("aformat"),//用"aformat"的AVFilter 创建一个FilterContext，并插入fg->graph
                                           name, args, NULL, fg->graph);
        if (ret < 0)
            return ret;

        ret = avfilter_link(last_filter, pad_idx, format, 0);//将last_filter做为src， format作为dst，串起来 src-->dst
        if (ret < 0)
            return ret;

        last_filter = format;//最后一个filterContext, 
        pad_idx = 0;//举例：执行结果 ofilter->filter：ff_asink_abuffer, last_filter和format:ff_af_anull
    }
    ...
    if (audio_volume != 256 && 0) {//05.
        ...
        AUTO_INSERT_FILTER("-vol", "volume", args);//插入filter volume
    }

    if (ost->apad && of->shortest) {//06.
        char args[256];
        int i;
        ...
        if (i<of->ctx->nb_streams) {
            ...
            AUTO_INSERT_FILTER("-apad", "apad", args);//插入filter apad
        }
    }

    snprintf(name, sizeof(name), "trim for output stream %d:%d",
             ost->file_index, ost->index);
    ret = insert_trim(of->start_time, of->recording_time,//07.插入trim 的filter
                      &last_filter, &pad_idx, name);
    if (ret < 0)
        return ret;
    //特别注意：前面都是以ff_af_anull为头，相互串起来的，才是将ofilter->filter即abuffersink 放在末尾
    if ((ret = avfilter_link(last_filter, pad_idx, ofilter->filter, 0)) < 0)//将last_filter做为src， ofilter->filter作为dst，串起来
        return ret;//举例ff_af_anull->ff_af_aformat 和 abuffersink ==>  ff_af_anull->ff_af_aformat->abuffersink 

    return 0;
}

int configure_output_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)//创建buffersink的AVFilterContext，用avfilter_link串起来，一般是null开始buffersink结束
{
    if (!ofilter->ost) {
...
int configure_filtergraph(FilterGraph *fg)//vs init_simple_filtergraph 只分配实例，并初始化相互指向
{
    ...
    for (cur = outputs, i = 0; cur; cur = cur->next, i++)
        configure_output_filter(fg, fg->outputs[i], cur);//tiger 主要函数  创建buffersink的AVFilterContext，用avfilter_link串起来，一般是null开始buffersink结束
    avfilter_inout_free(&outputs);//?
    //10. TODO: CONFIG
