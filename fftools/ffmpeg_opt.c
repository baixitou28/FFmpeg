...
static AVCodec *choose_decoder(OptionsContext *o, AVFormatContext *s, AVStream *st)//TIGER  先按codec_name 再按st->codecpar->codec_id查找AVCodec
{
    char *codec_name = NULL;

    MATCH_PER_STREAM_OPT(codec_names, str, codec_name, s, st);
    if (codec_name) {//如果有codec_name
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)//创建流InputStream* ist和解码上下文ist->dec_ctx ，并进行参数设置， ist和ic->streams[i]绑定
{//将ist放入全局数组input_streams
    ...
    for (i = 0; i < ic->nb_streams; i++) {//逐一处理每个流
        ...
        InputStream *ist = av_mallocz(sizeof(*ist));//分配内存
        ...
        if ((o->video_disable && ist->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ||
            ...
            (o->data_disable && ist->st->codecpar->codec_type == AVMEDIA_TYPE_DATA))
                ist->user_set_discard = AVDISCARD_ALL;//用户自定义禁止音频或视频
        //跳过几帧
        if (discard_str && av_opt_eval_int(&cc, discard_opt, discard_str, &ist->user_set_discard) < 0) {//是否要跳过几帧
        ...
        case AVMEDIA_TYPE_VIDEO://07.01 视频需要更多的处理
            if(!ist->dec)//如果没有解码器
                ist->dec = avcodec_find_decoder(par->codec_id);
#if FF_API_LOWRES
            if (st->codec->lowres) {//如果处理低像素
            ...
            // avformat_find_stream_info() doesn't set this for us anymore.//tiger program 自己写需要！！
            ist->dec_ctx->framerate = st->avg_frame_rate;

            MATCH_PER_STREAM_OPT(frame_rates, str, framerate, ic, st);
            if (framerate && av_parse_video_rate(&ist->framerate,//帧率
                                                 framerate) < 0) {
            ...
            if (hwaccel) {//硬件加速
                ...
                if (!strcmp(hwaccel, "nvdec"))//是NVIDIA的解码吗
                    hwaccel = "cuda";
                ...
                if (!strcmp(hwaccel, "none"))
                    ...
                    int i;
                    for (i = 0; hwaccels[i].name; i++) {//名称对比
                    ...
                    if (!ist->hwaccel_id) {
                        type = av_hwdevice_find_type_by_name(hwaccel);//查找设备
                    ...
                    if (!ist->hwaccel_id) {//找不到，打印所有硬件加速设备并退出。
            ...
            MATCH_PER_STREAM_OPT(hwaccel_devices, str, hwaccel_device, ic, st);//硬件设备
            if (hwaccel_device) {
            ...
            MATCH_PER_STREAM_OPT(hwaccel_output_formats, str,//硬件输出格式
            ...
            ist->hwaccel_pix_fmt = AV_PIX_FMT_NONE;//像素不用设置

            break;
        case AVMEDIA_TYPE_AUDIO://07.02
            ist->guess_layout_max = INT_MAX;
            MATCH_PER_STREAM_OPT(guess_layout_max, i, ist->guess_layout_max, ic, st);
            guess_input_channel_layout(ist);
            break;
        case AVMEDIA_TYPE_DATA:
        case AVMEDIA_TYPE_SUBTITLE: {//07.03
...
//TIGER 被open_files作为函数指针调用 //第一参数是解析后的可选项，第二参数是原始的组信息
static int open_input_file(OptionsContext *o, const char *filename)//重点函数：创建AVFormatContext，avformat_open_input 打开，add_input_streams,最后创建InputFile f 放在全局input_files
{//可能使用avforamt_find_stream_info再使用 //open_input_file主要聚焦：编码格式，音视频的主要参数，如frame rate等查找和设置 vs  open_output_file 关注filter等
    InputFile *f;
    AVFormatContext *ic;
    ...
    //02.可选项的格式
    if (o->format) {
        if (!(file_iformat = av_find_input_format(o->format))) {//简单查找格式
    ...
    //04.主要功能创建AVFormatContext ic，并从可选项中取参数来设置
    ...
    if (video_codec_name)
        ic->video_codec    = find_codec_or_die(video_codec_name   , AVMEDIA_TYPE_VIDEO   , 0);//找视频编码或解码，
    if (audio_codec_name)
    ...
    ic->video_codec_id     = video_codec_name    ? ic->video_codec->id    : AV_CODEC_ID_NONE;//如果找到，
    ...
    err = avformat_open_input(&ic, filename, file_iformat, &o->g->format_opts);//08.尝试打开filename
    if (err < 0) {
    ...
    for (i = 0; i < ic->nb_streams; i++)
        choose_decoder(o, ic, ic->streams[i]);//09.先按codec_name 再按st->codecpar->codec_id查找AVCodec
    //10. 如果设置了流模式查找信息，打开文件或者流来设置参数
    if (find_stream_info) {
        AVDictionary **opts = setup_find_stream_info_opts(ic, o->g->codec_opts);//10.01 可选项
        int orig_nb_streams = ic->nb_streams;

        /* If not enough info to get the stream parameters, we decode the
           first frames to get it. (used in mpeg case for example) */
        ret = avformat_find_stream_info(ic, opts);//10.02 查找
        //10.03 释放可选项
    ...
    if (o->start_time != AV_NOPTS_VALUE) {
        ...
        ret = avformat_seek_file(ic, -1, INT64_MIN, seek_timestamp, seek_timestamp, 0);//跳转
        if (ret < 0) {
    ...
    add_input_streams(o, ic);//创建流InputStream* ist和解码上下文ist->dec_ctx ，并进行参数设置， ist和ic->streams[i]绑定 

    /* dump the file content */
    av_dump_format(ic, nb_input_files, filename, 0);//TIGER AAC 在这里打印输出的aac 为LC模式即LOW不是MAIN
    //13.分配InputFile的内存，并设置InputFile f
    GROW_ARRAY(input_files, nb_input_files);//nb_input_files+1扩展，并自增
    f = av_mallocz(sizeof(*f));//分配实例
    if (!f)
        exit_program(1);
    input_files[nb_input_files - 1] = f;//14.放在全局数组的input_files中
    //逐一设置 InputFile f
    f->ctx        = ic;//相互指向
...
#if HAVE_THREADS
    f->thread_queue_size = o->thread_queue_size > 0 ? o->thread_queue_size : 8;//默认是8
#endif
    //15.获取没有用过的option
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)//创建好的ost放在全局数组output_streams里  
{
    ...
    //02. 可选项
    MATCH_PER_STREAM_OPT(frame_rates, str, frame_rate, oc, st);//02.01 frame_rates
    ...
    MATCH_PER_STREAM_OPT(frame_aspect_ratios, str, frame_aspect_ratio, oc, st);//02.02 frame_aspect_ratios
    ...
    MATCH_PER_STREAM_OPT(filter_scripts, str, ost->filters_script, oc, st);//02.03 filters
...
{//没有complex filter和nb_stream_maps的主要过程: 1.创建并初始化OutputFile 2.new_video_stream ，new_audio_stream， 3. init_simple_filtergraph 4. filter的关键参数 从ost中获取
    ...
    //02.在全局的output_files的数组里面增加一个输出文件OutputFile of
    GROW_ARRAY(output_files, nb_output_files);//扩展数组
    of = av_mallocz(sizeof(*of));//分配OutputFile 实例
    if (!of)
        exit_program(1);
...
loop_end:
    ...
    //09.校验 copy的时候不需要flags参数
#if FF_API_LAVF_AVCTX
    for (i = nb_output_streams - oc->nb_streams; i < nb_output_streams; i++) { //for all streams of this output file
        AVDictionaryEntry *e;
        ost = output_streams[i];//其他的传输流OutputStream

        if ((ost->stream_copy || ost->attachment_filename)
            && (e = av_dict_get(o->g->codec_opts, "flags", NULL, AV_DICT_IGNORE_SUFFIX))
            && (!e->key[5] || check_stream_specifier(oc, ost->st, e->key+6)))
            if (av_opt_set(ost->st->codec, "flags", e->value, 0) < 0)//不能含有flags等参数
                exit_program(1);
    }
#endif
    //10. 异常校验：输出没有流
    ...
    for (i = 0; i < o->nb_metadata; i++) {
        ...
        *val++ = 0;
        ...
        if (type == 's') {//22.03. 将元数据分别保存在oc->metadata，&oc->streams[j]->metadata， oc->chapters[index]->metadata，oc->programs[index]->metadata
...
static int open_files(OptionGroupList *l, const char *inout,//用open_file打开多个文件：解析组的可选项，再用委托open_file打开文件
...
int ffmpeg_parse_options(int argc, char **argv)//重要函数，因为这里包含了文件打开，所以相对复杂
{
    ...
    ret = parse_optgroup(NULL, &octx.global_opts);//02.校验分组的选项：
    if (ret < 0) {
    ...
    /* open input files */
    ret = open_files(&octx.groups[GROUP_INFILE], "input", open_input_file);//04.重要：打开所有输入文件：解析参数，并用open_input_file打开多个输入文件， open_input_file是函数！而且是很长的函数
    if (ret < 0) {
    ...
    ret = init_complex_filters();//05.初始化filter_complex选项：(后面好像还是用到了filter? ==>  真正初始化filter在 * //transcode_step-- > prcess_input-- > prcess_input_packet-- > decode_video-- > send_frame_to_filter-- > ifilter_send_frame-- > configure_filtergraph-- > configure_output_video_filter)
    if (ret < 0) {
    ...
    /* open output files */
    ret = open_files(&octx.groups[GROUP_OUTFILE], "output", open_output_file);//06.重要：打开所有输出文件：解析参数，并用open_output_file打开多个输出文件， open_output_file是函数！而且是很长的函数
    if (ret < 0) {
