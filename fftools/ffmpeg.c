...
sigterm_handler(int sig)
{
    ...
    received_nb_signals++;
    term_exit_sigsafe();
    if(received_nb_signals > 3) {//得多按两次
...
static int ifilter_send_frame(InputFilter *ifilter, AVFrame *frame)//发送数据 transcode_step-->process_input-->process_input_packet-->decode_video-->send_frame_to_filter-->ifilter_send_frame-->av_buffersrc_add_frame_flags-->av_buffersrc_add_frame_internal-->request_frame + ff_filter_activate
{
...
static int decode(AVCodecContext *avctx, AVFrame *frame, int *got_frame, AVPacket *pkt)//tiger 看注释
{
    int ret;
    ...
    *got_frame = 0;
    ...
    if (pkt) {
        ret = avcodec_send_packet(avctx, pkt);//送一个AVPacket
    ...
    ret = avcodec_receive_frame(avctx, frame);//取AVFrame
...
//配置graph transcode_step-->process_input-->process_input_packet-->decode_video-->send_frame_to_filter-->ifilter_send_frame-->configure_filtergraph-->configure_output_video_filter
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output, int64_t *duration_pts, int eof,//发送数据 transcode_step-->process_input-->process_input_packet-->decode_video-->send_frame_to_filter-->ifilter_send_frame-->av_buffersrc_add_frame_flags-->av_buffersrc_add_frame_internal-->request_frame + ff_filter_activate
                        int *decode_failed)//TIGER DECODE //TIGER IMPORTANT
{
    ...
    //02. TIGER IMPORTANT 这里就是常用代码  //tiger program
    update_benchmark(NULL);
    ret = decode(ist->dec_ctx, decoded_frame, got_output, pkt ? &avpkt : NULL);
    update_benchmark("decode_video %d.%d", ist->file_index, ist->st->index);//也许我们可以看这个
    if (ret < 0)
        *decode_failed = 1;
    //03.
    ...
    *duration_pts = decoded_frame->pkt_duration;
    ...
    //09.发送解析完成的decoded_frame到filter  //配置 send_frame_to_filter-->ifilter_send_frame-->configure_filtergraph-->configure_output_video_filter
    err = send_frame_to_filters(ist, decoded_frame); //数据send_frame_to_filter-->ifilter_send_frame-- > av_buffersrc_add_frame_flags-- > av_buffersrc_add_frame_internal-- > request_frame + ff_filter_activate
