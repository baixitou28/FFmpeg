...
{
    static const enum AVSampleFormat sample_fmts[] = {//01.只支持16位！！
...
static int filter_frame(AVFilterLink *inlink, AVFrame *samples)//TIGER 统计
{
    ...
    for (plane = 0; plane < nb_planes; plane++) {//02.统计，有符号16位转为无符号统计
...
//为什么是91？60DB是0.001 (10^(-60/20）)，90dB是2.81*10^(-5)
#define MAX_DB 91
...
static void print_stats(AVFilterContext *ctx)
{
    ...
    /* If nb_samples > 1<<34, there is a risk of overflow in the
       ...
       errors. */
    shift = av_log2(nb_samples >> 33);//02. 33是如何得出的？//TODO:为什么不是32？
    for (i = 0; i < 0x10000; i++) {//03.2^17即17位，histogram是uint64_t是64位的值，判断一下总数。shift最大为16位
    ...
    if (!nb_samples_shift)//04.
        return;
    power = (power + nb_samples_shift / 2) / nb_samples_shift;//05.还要除以nb_samples_shift，四舍五入
    ...
    for (; i <= MAX_DB && sum < nb_samples / 1000; i++) {//09.打印不为零的数据，这里的i是声音0，即有符号数的0，为什么这样因为统计加了0x8000
        av_log(ctx, AV_LOG_INFO, "histogram_%ddb: %"PRId64"\n", i, histdb[i]);//注意：实际运行，这些统计值和nb_samples相差比较大，估计是声音较小的比较多，较大应该没这么大的声音
        sum += histdb[i];
    }
}
