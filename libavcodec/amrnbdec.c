...
static int amrnb_decode_frame(AVCodecContext *avctx, void *data,
                              int *got_frame_ptr, AVPacket *avpkt)//tiger 主函数
{
    ...
    /* get output buffer */
    frame->nb_samples = AMR_BLOCK_SIZE;//固定长度
    if ((ret = ff_get_buffer(avctx, frame, 0)) < 0)//01.取帧
        return ret;
    buf_out = (float *)frame->data[0];//暴露数据地址

    p->cur_frame_mode = unpack_bitstream(p, buf, buf_size);//02.
    ...
    if (p->cur_frame_mode == MODE_12k2) {//03.
        lsf2lsp_5(p);
    } else
        lsf2lsp_3(p);

    for (i = 0; i < 4; i++)
        ff_acelp_lspd2lpc(p->lsp[i], p->lpc[i], 5);//04.

    for (subframe = 0; subframe < 4; subframe++) {//05.
        const AMRNBSubframe *amr_subframe = &p->frame.subframe[subframe];

        decode_pitch_vector(p, amr_subframe, subframe);//05.01

        decode_fixed_sparse(&fixed_sparse, amr_subframe->pulses,//05.02
        ...
        decode_gains(p, amr_subframe, p->cur_frame_mode, subframe,//05.03
                     &fixed_gain_factor);

        pitch_sharpening(p, subframe, p->cur_frame_mode, &fixed_sparse);//05.04

        if (fixed_sparse.pitch_lag == 0) {
            av_log(avctx, AV_LOG_ERROR, "The file is corrupted, pitch_lag = 0 is not allowed\n");
            return AVERROR_INVALIDDATA;
        }
        ff_set_fixed_vector(p->fixed_vector, &fixed_sparse, 1.0,//05.05
                            AMR_SUBFRAME_SIZE);

        p->fixed_gain[4] =//05.06
        ...
        ff_set_fixed_vector(p->excitation, &fixed_sparse, p->fixed_gain[4],//05.07
                            AMR_SUBFRAME_SIZE);
        ...
        for (i = 0; i < AMR_SUBFRAME_SIZE; i++)
            p->excitation[i] = truncf(p->excitation[i]);//05.08
        ...
        synth_fixed_gain = fixed_gain_smooth(p, p->lsf_q[subframe],//05.09
                                             p->lsf_avg, p->cur_frame_mode);

        synth_fixed_vector = anti_sparseness(p, &fixed_sparse, p->fixed_vector,//05.10
                                             synth_fixed_gain, spare_vector);

        if (synthesis(p, p->lpc[subframe], synth_fixed_gain,//05.11
        ...
        postfilter(p, p->lpc[subframe], buf_out + subframe * AMR_SUBFRAME_SIZE);//05.12

        // update buffers and history
        ff_clear_fixed_vector(p->fixed_vector, &fixed_sparse, AMR_SUBFRAME_SIZE);//05.13
        update_state(p);//05.14
    }

    p->acelpf_ctx.acelp_apply_order_2_transfer_function(buf_out,//06.
    ...
    p->acelpv_ctx.weighted_vector_sumf(p->lsf_avg, p->lsf_avg, p->lsf_q[3],//07.
                            0.84, 0.16, LP_FILTER_ORDER);

    *got_frame_ptr = 1;

    /* return the amount of bytes consumed if everything was OK */
    return frame_sizes_nb[p->cur_frame_mode] + 1; // +7 for rounding and +8 for TOC//08.
}
